...
<template>
  <header class="header flex items-center justify-between py-2.5 sm:py-4 md:py-5 px-3 sm:px-6 md:px-8 my-2 sm:my-4 md:my-5 bg-card-bg rounded-xl sm:rounded-2xl shadow-sm gap-2 sm:gap-4">
    <!-- Mobile Menu Toggle -->
    <button 
    ...
    <div class="header-actions flex items-center gap-1 sm:gap-2 md:gap-3 flex-shrink-0">
      ...
      <button class="lang-btn hidden lg:flex items-center gap-1.5 px-3 py-2 text-sm font-medium text-text-light hover:text-primary transition-colors">
      ...
      <div class="user-profile-wrapper relative" @click.stop>
        ...
        <!-- Profile Dropdown -->
        <div 
          :class="['profile-dropdown absolute top-[calc(100%+8px)] sm:top-[calc(100%+12px)] right-0 bg-card-bg rounded-xl shadow-lg min-w-[200px] sm:min-w-[220px] transition-all duration-200 z-50 border border-border', showProfileDropdown ? 'active' : 'opacity-0 invisible -translate-y-2.5']"
        >
  ...
  >
    <div class="flex items-center gap-3 bg-background px-3 py-2.5 rounded-lg border border-transparent focus-within:border-primary transition-colors">
      <i class="fa-solid fa-magnifying-glass text-text-light text-sm"></i>
      <input 
        v-model="searchQuery"
        type="text" 
      ...
      ></i>
      <button 
        class="text-text-light hover:text-text text-sm ml-1"
        @click="toggleMobileSearch"
      >
        Cancel
      </button>
